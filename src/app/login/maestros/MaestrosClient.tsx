...
export default function MaestrosClient({ cedula: cedulaProp }: { cedula?: string }) {
  ...
  // Habilita logs pasando ?rtlog=1 o ?debug=1
  ...
  /* ====== Realtime SIN polling (con UPDATE inteligente y transition_log) ====== */
  useEffect(() => {
    if (!asig) return;

    // Parche fino al INSERT de progreso (evita fetch completo)
    const tryPatchProgresoInsert = async (row: any) => {
      ...
      // Traer persona para render inmediato
    ...
    // Refresco silencioso (debounce 150 ms)
    let t: number | null = null;
    const refreshQuiet = () => {
      rtLog('refreshQuiet: schedule');
      if (t) window.clearTimeout(t);
      t = window.setTimeout(async () => {
        rtLog('refreshQuiet: flush');
        const d = diaRef.current as Dia | null;
        const s = semanaRef.current as Semana;
        if (d) {
          rtLog('refreshQuiet: fetch', { semana: s, dia: d });
    ...
    rtLog('channel:create', channelName);
    const ch = supabase.channel(channelName);
    // Solo logs de diagnóstico (no alteran la lógica)
    ch.on('postgres_changes', { event: '*', schema: 'public', table: 'progreso' }, (p: any) => {
      rtLog('ev:progreso', p?.eventType, { old: p?.old?.id, new: p?.new?.id });
    });
    ch.on('postgres_changes', { event: '*', schema: 'public', table: 'llamada_intento' }, (p: any) => rtLog('ev:llamada_intento', p?.eventType));
    ch.on('postgres_changes', { event: '*', schema: 'public', table: 'asistencia' }, (p: any) => rtLog('ev:asistencia', p?.eventType));
    ch.on('postgres_changes', { event: '*', schema: 'public', table: 'persona' }, (p: any) => rtLog('ev:persona', p?.eventType));
    ch.on('postgres_changes', { event: '*', schema: 'public', table: 'asignaciones_maestro' }, (p: any) => rtLog('ev:asignaciones_maestro', p?.eventType));

    // Progreso
    ch.on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'progreso' }, (payload) => { tryPatchProgresoInsert(payload.new); });
  ...
  if (row) {
    // fuerza simulación de UPDATE con old=null
    await tryPatchProgresoUpdate({}, row);
  }
});

    ch.on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'progreso' }, (payload) => {
      // Si tenemos old/new (REPLICA IDENTITY FULL) parchamos fino; si no, fallback a refresh
    ...
    // Llamada / Asistencia / Persona / Asignación → refresh silencioso
    ...
    // Transición registrada (por si el UPDATE vino “corto”)
    ch.on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'transition_log' }, () => refreshQuiet());

    ch.subscribe((status) => {
      rtLog('channel:status', status);
      if (status === 'SUBSCRIBED') { rtLog('channel:subscribed -> first refresh'); refreshQuiet(); }
    });

    return () => {
      rtLog('channel:unsubscribe');
      ch.unsubscribe();
      if (t) window.clearTimeout(t);
