...
/* ========= Config etapa (usar vistas por etapa) ========= */
const V_PENDIENTES = 'v_devocionales_llamadas_pendientes_hist'; // ← histórica (llamada1/2/3)
const V_AGENDADOS = 'v_devocionales_agendados';
        ...
        <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            <div className="relative w-[min(680px,92vw)] max-h-[90vh] overflow-auto rounded-[24px] bg-white shadow-[0_16px_40px_rgba(0,0,0,.22)] ring-1 ring-black/10">
                ...
                <div className="p-7 md:p-9">
                    ...
                    <div className="mt-2 text-neutral-500">{dev ? `${CARD_TITLE} ${dev}` : ''}</div>
...
function AttendancePanel({
    ...
    const load = useCallback(async () => {
        ...
        const { data, error } = await supabase
            .from(V_AGENDADOS) // vista por etapa
            ...
            <div className="flex items-center justify-between px-4 md:px-6 py-3 bg-[linear-gradient(135deg,#eaf3ff,#f8f1ff)]">
                <div>
                    <h3 className="text-[15px] md:text-base font-semibold text-neutral-900">
                        Listado Estudiantes Día {day} — {CARD_TITLE} {modulo}
                    </h3>
...
/* ============================= Página ============================= */
export default function PageDevocionales() {
    const [open, setOpen] = useState(false);
    const [dev, setDev] = useState<DevKey | null>(null);
    const [mode, setMode] = useState<Mode>('default');

    const [selectedDevCard, setSelectedDevCard] = useState<DevKey | null>(null);
    ...
    const loadPendientes = useCallback(
        async (modulo: DevKey, semana: Week, dia: Day) => {
            ...
            // Usamos la vista histórica por etapa y permitimos dia NULL
            const { data, error } = await supabase
                .from(V_PENDIENTES)
    ...
    useEffect(() => {
        ...
        if (!dev || !activeWeek || !activeDay) return;
        void loadPendientes(dev, activeWeek, activeDay);
    }, [mode, contactView, dev, activeWeek, activeDay, loadPendientes]);

    const handleOpen = (d: DevKey) => {
        setSelectedDevCard(d);
        setDev(d);
        setOpen(true);
    };
