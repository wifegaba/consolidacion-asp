...
export default function BienvenidaClient({ cedula: cedulaProp }: { cedula?: string }) {
  ...
  // Cédula solo por props (inyectada desde Server Component leyendo cookie segura)
  const cedula: string = cedulaProp ?? "";
  ...
  if (loading) {
    return (
      <main className="min-h-screen grid place-items-center bg-gradient-to-br from-gray-50 via-white to-blue-100">
        <div className="bg-white/40 backdrop-blur-xl p-8 rounded-2xl shadow-xl">Cargando…</div>
      </main>
    );
  }
    ...
    return (
      <main className="min-h-screen grid place-items-center bg-red-50">
        <div className="bg-white p-6 rounded-xl shadow-lg text-center">
          ...
          <button className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg" onClick={() => router.replace("/login")}>
            Volver al login
          </button>
        </div>
      </main>
  ...
  return (
    <main
      className="relative min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 via-white to-blue-100 overflow-hidden"
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-lg bg-white/30 backdrop-blur-2xl border border-white/40 rounded-3xl shadow-2xl p-10 text-center">
        ...
        <div className="mt-8 grid gap-4">
          {rol === "contactos" && (
            <button className="w-full bg-blue-600 text-white py-4 rounded-lg hover:bg-blue-700 transition" onClick={() => router.push(`/login/contactos1`)}>
              Llamadas pendientes
            </button>
          )}

          {rol === "maestro" && (
            <button className="w-full bg-green-600 text-white py-4 rounded-lg hover:bg-green-700 transition" onClick={() => router.push(`/login/maestros`)}>
              Asistencias de hoy
            </button>
          )}
        </div>
