...
import AsistenciasSection from '../admin/components/AsistenciasSection'
import { type KidsCoordinador } from '../admin/components/CoordinadorModal'

export default function KidsAsistenciasPage() {
  ...
  /* ── Auth: coordinador primero, luego admin ── */
  useEffect(() => {
    async function load() {
      try {
  ...
  const usuario = coord
    ? { nombre: coord.nombre, apellido: coord.apellido }
    : null

  return (
    <>
    <style>{`
      @keyframes asistFadeIn {
    ...
    <div style={{
      ...
      position:'relative',
      animation:'asistFadeIn .42s cubic-bezier(0.25,0.46,0.45,0.94) both',
    }}>
      ...
      <div style={{
        ...
        >
          <div style={{
            ...
            <AsistNavIcon type="coord" active={false} />
          </div>
        ...
        >
          <div style={{
            ...
            <AsistNavIcon type="salir" active={false} />
          </div>
        ...
        >
          <AsistLogoCircle size={96} />
        </div>

        {/* Derecha 1: Niños */}
        <div
          onClick={() => { setLogoNavOpen(false); router.push('/kids/ninos') }}
          style={{
        ...
        >
          <div style={{
            ...
            <AsistNavIcon type="ninos" active={false} />
          </div>
          <span style={{ fontSize:9, fontWeight:700, color:'#6b7280', whiteSpace:'nowrap' }}>Niños</span>
        </div>

        {/* Derecha 2: Asistencias (activo) */}
        <div
        ...
        >
          <div style={{
            ...
            <AsistNavIcon type="asistencias" active={true} />
          </div>
          <span style={{ fontSize:9, fontWeight:700, color:'#7c3aed', whiteSpace:'nowrap' }}>Asist.</span>
        </div>
...
/* ── Logo circle ── */
function AsistLogoCircle({ size }: { size: number }) {
...
/* ── NavIcons ── */
function AsistNavIcon({ type, active }: { type: 'coord' | 'ninos' | 'asistencias' | 'salir'; active: boolean }) {
