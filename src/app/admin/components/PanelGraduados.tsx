...
import { Award, Search, GraduationCap, Medal, Star, User, Loader2 } from 'lucide-react';
...
export default function PanelGraduados({ estudiantes, inscripciones, fotoUrls, maestros, cursos, onDataUpdated, currentUser }: PanelGraduadosProps) {
                    ...
                    >
                        {graduados.map(g => (
                            <motion.div key={g.id} variants={LIST_ITEM_VARIANTS}>
                                <GraduadoCard graduado={g} fotoUrl={g.foto_path ? fotoUrls[g.foto_path] : undefined} onClick={() => setSelectedStudent(g)} />
                            </motion.div>
                        ))}
                    </motion.div>
                )}
                    ...
                    >
                        <div className="flex-1 w-full overflow-hidden p-0 bg-slate-50 flex flex-col min-h-0 relative z-[201]">
                            <HojaDeVidaPanel
                                ...
                                signedUrl={selectedStudent.foto_path ? fotoUrls[selectedStudent.foto_path] : null}
                                onUpdated={() => {
...
function GraduadoCard({ graduado, fotoUrl, onClick }: { graduado: any, fotoUrl?: string, onClick: () => void }) {
    const avatar = fotoUrl || generateAvatar(graduado.nombre);

    return (
        <div 
            className="group relative w-full h-[320px] rounded-[2rem] p-[2px] overflow-hidden cursor-pointer"
            onClick={onClick}
        >
