...
export default function AdminPage() {
  ...
  return (
    <div className="relative h-[100dvh] w-full overflow-hidden bg-[#bfdbfe] text-gray-900 font-sans">
       {/* FONDO ESTATICO */}
       <div className="absolute inset-0 z-0 bg-gradient-to-b from-[#bfdbfe] to-[#c4b5fd]" />
               ...
               <div className="flex-1 overflow-y-auto p-2 md:p-8 scroll-smooth">
                 <AnimatePresence mode="wait">
                   <motion.div key={activeTab} variants={fadeTransition} initial="hidden" animate="visible" exit="exit" className="h-full flex flex-col">
                      {activeTab === 'servidores' && <GestionServidores />}
...
function ModalCrearEditarMaestro({ maestroInicial, onClose, onSuccess }: { maestroInicial: MaestroConCursos | null, onClose: ()=>void, onSuccess: ()=>void }) {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try { await supabase.rpc('fn_guardar_servidor_con_rol', { p_cedula: ced, p_nombre: nom, p_rol: rol, p_telefono: null, p_email: null }); onSuccess(); }
    ...
    <ModalTemplate onClose={onClose} title={isEdit ? "Editar Maestro" : "Nuevo Maestro"}>
       <form onSubmit={save} className="p-6 space-y-4 flex-1 overflow-y-auto">
          ...
          <FormSelect label="Rol" value={rol} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setRol(e.target.value)}>
             {['Coordinador', 'Timoteo', 'Logistica', 'Director', 'Maestro Ptm', 'Administrador'].map(r => <option key={r} value={r}>{r}</option>)}
          </FormSelect>
          <div className="pt-4 flex justify-end gap-2">
             ...
             <button disabled={loading} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-lg shadow-blue-500/30">{loading ? "Guardando..." : "Guardar"}</button>
          </div>
       </form>
    </ModalTemplate>
  );
...
function ModalAsignarCursos({ maestro, cursosDisponibles, onClose, onSuccess }: { maestro: MaestroConCursos, cursosDisponibles: Curso[], onClose: ()=>void, onSuccess: ()=>void }) {
   ...
   return (
    <ModalTemplate onClose={onClose} title={`Cursos: ${maestro.nombre}`}>
       ...
       <div className="p-4 border-t border-white/50 flex justify-end"><button onClick={save} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold">Guardar Cambios</button></div>
    </ModalTemplate>
   );
}
