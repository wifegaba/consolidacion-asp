...
function DarkRowDate({ label, value, edit, onChange, icon }: any) {
  ...
  return (
    <div className="flex flex-row items-center justify-between py-3 border-b border-slate-200 last:border-0 gap-2 hover:bg-white/40 px-3 -mx-3 rounded-lg transition-colors">
      <div className="flex items-center gap-2 text-xs font-medium text-slate-600 uppercase w-auto sm:w-[160px] flex-shrink-0 tracking-wide mt-0.5">
        {icon && <span className="opacity-50">{icon}</span>}
        {label}
      </div>
      <div className="text-sm text-slate-700 font-light flex-1 text-right break-words leading-relaxed relative">
        {edit ? (
          <div className="relative inline-block w-full sm:w-auto">
            <input
              ...
              onChange={(e) => onChange(e.target.value)}
              className="w-full sm:w-auto text-sm text-right border-b border-blue-500/50 px-2 py-1 focus:border-blue-400 transition-colors bg-white/50 rounded-t-sm outline-none text-slate-700 font-medium appearance-none min-h-[30px] shadow-sm cursor-pointer"
            />
          </div>
        ) : (
