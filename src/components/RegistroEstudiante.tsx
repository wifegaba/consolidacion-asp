...
function SemesterPanel({
    ...
    const handleGuardar = async () => {
        try {
            ...
            onUpdated?.(); // 👈 avisa al padre para refrescar tarjetas
            onClose();
        } catch (e) {
...
export default function Page() {
    ...
    // 👇 NUEVO: tick para refrescar resumen tras guardar notas
    ...
    useEffect(() => {
        ...
        const run = async () => {
            ...
            // Respuesta instantánea desde caché (normalizada)
    ...
    // Cargar resumen (se refresca también cuando cambia resumenTick)
    useEffect(() => {
        let alive = true;
        (async () => {
            if (!sel?.id) {
    ...
    }, [sel?.id, resumenTick]); // 👈 depende del tick
