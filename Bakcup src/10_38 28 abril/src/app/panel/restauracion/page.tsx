...
/* ========= Config etapa ========= */
const ETAPA = 'Restauración';
const CARD_TITLE = 'Restauración';

// Vistas por etapa (si existen). Hay fallback automático si no existen.
const V_PENDIENTES_STAGE = 'v_restauracion_llamadas_pendientes_hist';
const V_AGENDADOS_STAGE = 'v_restauracion_agendados';

// Vistas/Tablas base para fallback
const V_PENDIENTES_BASE = 'v_llamadas_pendientes_hist'; // histórica sin 'etapa'
const V_AGENDADOS_BASE = 'v_agendados';                 // tiene 'etapa'
const T_PROGRESO = 'progreso';                           // para filtrar por etapa cuando usemos la HIST
...
const grad = 'from-[#7EA1FF] via-[#88A9FF] to-[#A3BDFF]'; // look & feel elegante
const MAC_ACCENT = {
...
function Modal({
    ...
    return (
        <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center p-6">
            <div aria-hidden className="fixed inset-0 bg-black/10 backdrop-blur-[2px]" onClick={onClose} />
            <div className="relative w-[min(680px,92vw)] max-h-[90vh] overflow-auto rounded-[24px] bg-white shadow-[0_16px_40px_rgba(0,0,0,.22)] ring-1 ring-black/10">
                ...
                <div className="p-7 md:p-9">
                    ...
                    <div className="mt-2 text-neutral-500">{res ? `${CARD_TITLE} ${res}` : ''}</div>
...
/* =============== Tarjeta Restauración (única y centrada) =============== */
function RestCard({
                      onClick,
                      selected = false,
                  }: {
    ...
    return (
        <section className="animate-cardIn rounded-[16px] bg-white shadow-[0_10px_28px_-14px_rgba(16,24,40,.28)] ring-1 ring-black/5">
            ...
            <ul className="divide-y divide-black/5">
                {items.map((c) => {
                    const telHref = c.tel ? `tel:${c.tel.replace(/\s+/g, '')}` : undefined;
                    return (
                        <li
                            key={c.id}
                        ...
                        >
                            <div className="flex items-start justify-between gap-4">
                                {/* Izquierda: nombre + teléfono con icono */}
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        {telHref ? (
                                            <a
                                                ...
                                                className="mt-0.5 inline-flex items-center gap-1.5 text-neutral-600 text-xs md:text-sm hover:text-neutral-800"
                                                aria-label={`Llamar a ${c.tel}`}
                                                title={c.tel ?? ''}
                                            >
                                                {/* phone icon */}
                                            ...
                                            <div className="mt-0.5 inline-flex items-center gap-1.5 text-neutral-500 text-xs md:text-sm">
                                                <svg viewBox="0 0 24 24" width="14" height="14" aria-hidden="true" className="opacity-50">
                                                    <path
                                                        d="M6.6 10.8c1.3 2.5 3.1 4.4 5.6 5.6l2.1-2.1a1 1 0 0 1 1.1-.22c1.2.48 2.6.74 4 .74a1 1 0 0 1 1 1v3.5a1 1 0 0 1-1 1C12.1 20.3 3.7 11.9 3.7 2.7a1 1 0 0 1 1-1H8.2a1 1 0 0 1 1 1c0 1.4.26 2.8.74 4a1 1 0 0 1-.22 1.1l-2.1 2.1Z"
                                                        fill="currentColor"
                                                    />
            ...
            <div className="mb-4 rounded-2xl ring-1 ring-black/5 bg-[linear-gradient(135deg,#eef3ff,#f6efff)] px-4 py-3 md:px-5 md:py-4 flex items-center justify-between gap-4">
                ...
                {telHref && (
                    <a
                        ...
                        aria-label={`Llamar a ${contact.tel}`}
                    >
...
/* =============== Panel de asistencias (Restauración) =============== */
function AttendancePanel({
    ...
    const load = useCallback(async () => {
        ...
        // Intento 1: vista por etapa
        let data: any[] | null = null;
        let error: any = null;

        const r1 = await supabase
            .from(V_AGENDADOS_STAGE)
        ...
        if (r1.error) {
            // Fallback: vista base + filtro por etapa
            const r2 = await supabase
                .from(V_AGENDADOS_BASE)
                .select('progreso_id,nombre,telefono,semana')
                .eq('etapa', ETAPA)
                .eq('modulo', modulo)
                .eq('dia', day)
    ...
    const enviar = async () => {
        ...
        try {
            for (const [progId, v] of entradas) {
                const { error } = await supabase.rpc(RPC_MARCAR_ASISTENCIA, {
                    p_progreso: progId,
                    p_asistio: v === 'A',
                });
            ...
            <div className="flex items-center justify-between px-4 md:px-6 py-3 bg-[linear-gradient(135deg,#eaf3ff,#f8f1ff)]">
                <div>
                    <h3 className="text-[15px] md:text-base font-semibold text-neutral-900">
                        Listado Estudiantes Día {day} — {CARD_TITLE} 1
                    </h3>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        onClick={onExit}
                        className="text-sm font-semibold rounded-full bg-white px-3 py-1.5 ring-1 ring-black/10 shadow-sm hover:shadow-md transition"
                    >
                        Salir
                    </button>
                </div>
...
/* ============================= Página ============================= */
export default function PageRestauracion() {
    const [open, setOpen] = useState(false);
    const [res, setRes] = useState<ResKey | null>(null);
    const [mode, setMode] = useState<Mode>('default');

    const [selectedResCard, setSelectedResCard] = useState<ResKey | null>(null);
    ...
    const loadPendientes = useCallback(
        async (modulo: ResKey, semana: Week, dia: Day) => {
            setLoadingPend(true);

            // Intento 1: vista histórica por etapa
            let data: any[] | null = null;
            let error: any = null;

            const r1 = await supabase
                .from(V_PENDIENTES_STAGE)
                .select('progreso_id,nombre,telefono,llamada1,llamada2,llamada3,dia')
                .eq('modulo', modulo)
                .eq('semana', semana)
                .or(`dia.eq.${dia},dia.is.null`)
                .order('nombre', { ascending: true });

            if (r1.error) {
                // Fallback profesional:
                //  - tomo la HIST base (sin etapa)
                //  - y filtro por etapa/activo cruzando con progreso (lista de IDs válidos)
                const rHist = await supabase
                ...
                if (rHist.error || rProg.error) {
                    data = [];
                    error = rHist.error ?? rProg.error;
                } else {
                    const allowed = new Set((rProg.data ?? []).map((x: any) => x.id));
                    data = (rHist.data ?? []).filter((row: any) => allowed.has(row.progreso_id));
                }
            } else {
                data = r1.data ?? [];
                error = r1.error;
            ...
            setPendientes((error ? [] : (data ?? [])) as PendienteRow[]);
    ...
    useEffect(() => {
        ...
        if (!res || !activeWeek || !activeDay) return;
        void loadPendientes(res, activeWeek, activeDay);
    }, [mode, contactView, res, activeWeek, activeDay, loadPendientes]);

    const handleOpen = () => {
        setSelectedResCard(1);
        setRes(1);
        setOpen(true);
    };
      ...
      `}
        ...
        >
            <div className="mx-auto w-full max-w-[1260px]">
                ...
                {/* Única tarjeta centrada */}
                <div className="w-full flex justify-center">
                    <RestCard onClick={handleOpen} selected={selectedResCard === 1} />
                </div>

                {/* CONTENIDO */}
                {mode === 'contactos' ? (
                    contactView === 'panel' && activeWeek && activeDay && res ? (
                        <>
                            <div className="mt-6 md:mt-7 flex items-baseline gap-3">
                                <h2 className="text-lg md:text-xl font-semibold text-neutral-900 animate-cardIn">
                                    Llamadas pendientes {CARD_TITLE} 1
                                </h2>
                  ...
                  Semana {activeWeek} • {activeDay}
                                ...
                                {selectedContact ? (
                                    <FollowUpForm
                                        ...
                                        onSubmit={async ({ resultado, notas }) => {
                                            if (!res || !activeWeek || !activeDay || !selectedContact) return;
                                            setSavingCall(true);
                                            try {
                                                ...
                                                await loadPendientes(1, activeWeek, activeDay);
                                                setSelectedProgreso(null);
                                            } catch (e: any) {
                    ...
                    <div className="mt-6 md:mt-7">
                        <AttendancePanel dia="Domingo" onExit={() => setMode('default')} />
                    </div>
                ) : (
                    <div className="mt-6 md:mt-7 grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-6">
                        <section className="rounded-[16px] bg-white shadow-[0_10px_28px_-14px_rgba(16,24,40,.28)] ring-1 ring-black/5">
                            ...
                            <div className="px-4 md:px-5 py-6 text-neutral-500 text-sm">
                                Abre la tarjeta y elige “Llamadas pendientes”.
                            </div>
                        </section>
                        ...
                        <section className="rounded-[16px] bg-white shadow-[0_10px_28px_-14px_rgba(16,24,40,.28)] ring-1 ring-black/5">
                            ...
                            <div className="px-4 md:px-5 py-6 text-neutral-500 text-sm">
                                Abre la tarjeta y elige “Asistencias”.
                            </div>
                        </section>
                    </div>
                )}
        ...
        .accent-line { background-size: 200% 100%; animation: accentMove 3.2s ease-in-out infinite; }
