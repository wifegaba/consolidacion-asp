...
export default function NinosSection({ usuario, logoNavOpen = false }: Props) {
  ...
  const [selectedNino,     setSelectedNino]     = useState<KidsNino | null>(null)
  ...
  const displayed = filtered.slice(0, VISIBLE_COUNT)
  ...
  return (
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex:1, minHeight:0, overflowY:'auto', paddingRight:4, paddingBottom: isMobile ? 90 : 76 }}>
          {loading ? (
              ...
              <div style={{
                ...
                {displayed.slice(0, 5).map((n, idx) => (
                  <NinoCard
                    ...
                    onSelect={() => setSelectedNino(n)}
                  />
                ))}
                {!isMobile && (
                  <div
                    className="lgx-content-card lgx-content-card--static"
                    aria-label={selectedNino ? `Información de ${selectedNino.nombre}` : 'Espacio reservado'}
                    style={{
                      gridColumn: '6 / span 2',
                      gridRow: '1 / span 2',
                      minHeight: 376,
                      ...
                      boxShadow: 'inset 0 1px rgba(255,255,255,.5), 0 12px 26px rgba(31,34,77,.08)',
                      backdropFilter: 'blur(18px) saturate(130%)',
                      WebkitBackdropFilter: 'blur(18px) saturate(130%)',
                    }}
                    >
                    {selectedNino && (
                      <div key={selectedNino.id} style={{ padding: 24, color: '#172033', animation: 'profileSwapIn .52s cubic-bezier(.16,1,.3,1) both' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                          <div style={{ width: 104, height: 104, borderRadius: '50%', overflow: 'hidden', flexShrink: 0, background: 'rgba(255,255,255,.5)', border: '2px solid rgba(255,255,255,.9)', boxShadow: '0 10px 24px rgba(31,34,77,.12)' }}>
                            {selectedNino.foto_url ? <img src={selectedNino.foto_url} alt={`${selectedNino.nombre} ${selectedNino.apellido}`} style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '50%', display: 'block' }} /> : <div style={{ width: '100%', height: '100%', display: 'grid', placeItems: 'center', fontSize: 24, fontWeight: 800, color: '#64748b' }}>{selectedNino.nombre.charAt(0)}{(selectedNino.apellido || '').charAt(0)}</div>}
                          </div>
                          <div>
                            <div style={{ fontSize: 11, fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '.12em' }}>Perfil del niño</div>
                            <div style={{ fontSize: 26, lineHeight: 1.08, fontWeight: 850, letterSpacing: '-.04em', marginTop: 7 }}>{selectedNino.nombre} {selectedNino.apellido}</div>
                          </div>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 8, marginTop: 18 }}>
                          <div style={{ padding: '8px 11px', borderRadius: 12, background: 'rgba(255,255,255,.38)', border: '1px solid rgba(255,255,255,.62)' }}><small style={{ color: '#64748b', fontWeight: 700, fontSize: 10 }}>Edad</small><strong style={{ display: 'block', fontSize: 15, marginTop: 3 }}>{selectedNino.edad ?? '—'} años</strong></div>
                          <div style={{ padding: '8px 11px', borderRadius: 12, background: 'rgba(255,255,255,.38)', border: '1px solid rgba(255,255,255,.62)' }}><small style={{ color: '#64748b', fontWeight: 700, fontSize: 10 }}>Grupo</small><strong style={{ display: 'block', fontSize: 15, marginTop: 3 }}>{selectedNino.grupo ?? '—'}</strong></div>
                          <div style={{ padding: '8px 11px', borderRadius: 12, background: 'rgba(255,255,255,.38)', border: '1px solid rgba(255,255,255,.62)' }}><small style={{ color: '#64748b', fontWeight: 700, fontSize: 10 }}>Acudiente</small><strong style={{ display: 'block', fontSize: 13, marginTop: 3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{selectedNino.nombre_acudiente || selectedNino.acudiente || '—'}</strong></div>
                          <div style={{ padding: '8px 11px', borderRadius: 12, background: 'rgba(255,255,255,.38)', border: '1px solid rgba(255,255,255,.62)' }}><small style={{ color: '#64748b', fontWeight: 700, fontSize: 10 }}>Teléfono</small><strong style={{ display: 'block', fontSize: 13, marginTop: 3 }}>{selectedNino.telefono_acudiente || selectedNino.telefono || '—'}</strong></div>
                        </div>
                        <div style={{ marginTop: 20, color: '#52637d', fontSize: 13, lineHeight: 1.5 }}>{selectedNino.observaciones || 'Sin observaciones registradas.'}</div>
                      </div>
                    )}
                  </div>
                )}
              ...
              {filtered.length > VISIBLE_COUNT && (
                <button
                  ...
                  style={{
                    ...
                    padding:isCompact ? '10px 0' : '12px 0', borderRadius:12,
                    border:'1px solid rgba(0,0,0,.07)',
                    background:'rgba(255,255,255,.75)',
                    fontSize:isCompact ? 11.5 : 12, fontWeight:600, color:'#6b7280',
...
/* ── NinoCard ────────────────────────────────────────────────────────────── */
function NinoCard({
  nino, idx, dense, onEdit, onToggle, onDelete, onActivateAI, onSaveObs, onSelect,
}: {
  nino:          KidsNino
  idx:           number
  dense?:        boolean
