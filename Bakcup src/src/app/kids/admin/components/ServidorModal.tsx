...
interface Props {
  ...
  launchOrigin?: { dx: number; dy: number } | null
}
...
export default function ServidorModal({ servidor, onClose, onSave, onSaved, launchOrigin }: Props) {
  ...
  const launchTransform = launchOrigin && !isEdit
    ? `translate(${launchOrigin.dx}px, ${launchOrigin.dy}px) scale(.10)`
    : isMobile
      ? 'translateY(100%)'
  ...
  const dialogStyle: React.CSSProperties = isMobile ? {
    ...
    transform: visible ? settledTransform : launchTransform,
    opacity: visible ? 1 : 0,
    transition:'transform .48s cubic-bezier(.16,1,.3,1), opacity .22s',
    display:'flex', flexDirection:'column', overflow:'hidden',
  }
